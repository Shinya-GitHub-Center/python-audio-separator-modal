...
既定モデル(`DEFAULT_MODEL`)には、楽器単位でのフル分解(vocals/drums/bass/guitar/piano/other の6-stem)ができる`htdemucs_6s.yaml`(Demucs v4)を採用した。2-stem(vocals/instrumental)のみで良い場合や、より高いSDR(分離精度)を優先したい場合は、`model_bs_roformer_ep_317_sdr_12.9755.ckpt`(viperx版BS-Roformer)のように`--model-filename`引数で他モデルに切り替え可能。利用可能なモデル一覧は`audio-separator --list_models`で確認できる。
