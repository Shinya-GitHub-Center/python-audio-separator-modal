...
Demucsより新しく現在も活発に開発されている（2026年時点で直近リリースあり、月次以上の頻度で更新）、かつ品質面でもTransformerベースのBS-Roformer/Mel-Band Roformer系モデル（SDR 12.9dB前後、Demucsの旧世代モデルより高精度）を扱える点が決め手。既定モデルには`model_bs_roformer_ep_317_sdr_12.9755.ckpt`（viperx版BS-Roformer、vocals/instrumental分離）を採用した。
